...
```
	...
	The log transformation succeeded in its goal of breaking up the lower GDP per Capita observations. However, it is likely that the strength of our model was decreased. It seems that after the log transformation, the observations broke into two “pools” separated by GDP per Capita. So while there are no outliers on this axis, the vast majority of observations in the lower pool fall below the regression line, and in the higher pool, most observations are above it. But the positive relationship remains, and now we would like to see if it has changed at all in the last 50 years across different regions in the world, or if this as always been the case.
